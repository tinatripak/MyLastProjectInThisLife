...
namespace d.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class TopFilmsOfYearController : ControllerBase
    {
        [HttpGet]
        public async Task<Titles[]> GetTitles()
        {
            JObject search = JObject.Parse(await Get($"https://api.themoviedb.org/3/movie/popular?api_key=1490553eadc577108121742dfafd35b2&language=en-US&page=1"));
